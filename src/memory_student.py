...
class StudentMemory:
    ...
    def retrieve_long_term(self, user_id: str, thread_id: str, query: str) -> str:
        # Context Block is computed from a current evaluation-thread slice.
        prime_eval_thread(self.client, user_id, thread_id, query)
        user_context = self.client.thread.get_user_context(thread_id=thread_id)
        context_block = getattr(user_context, "context", "") or ""

        # Keep the optional evidence search scoped to this user graph.  Render
        # edges into auditable text so validity/provenance survives serialization.
        try:
            ...
            fact_text = ""
        return join_nonempty([str(context_block), fact_text], sep="\n\n")
    ...
    def retrieve_semantic(self, graph_id: str, query: str) -> str:
        ...
        try:
            ...
            # Some account/SDK combinations do not expose episode search for
            # standalone graphs; nodes are the compatible fallback.
            results = self.client.graph.search(
                graph_id=graph_id,
                query=cap_query(query),
                scope="nodes",
